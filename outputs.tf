...
output "SQS-QUEUE" {
  value       = aws_sqs_queue.mux_webhook_sqs.id
  description = "The SQS Queue URL"
}
output "APIGW-URL" {
  value       = aws_api_gateway_stage.muxWebhookGatewayStage.invoke_url
  description = "The API Gateway Invocation URL Queue URL"
}

# Command for testing to send data to api gateway
output "Test-Command1" {
  value       = "curl --location --request POST '${aws_api_gateway_stage.muxWebhookGatewayStage.invoke_url}/webhook' --header 'Content-Type: application/json'  --data-raw '{ \"TestMessage\": \"Hello From ApiGateway!\" }'"
...
output "Test-Command2" {
  value       = "aws sqs receive-message --queue-url ${aws_sqs_queue.mux_webhook_sqs.id}"
...
#   value       = aws_cloudwatch_log_group.mux_webhook_eventbridge_log_group.id
